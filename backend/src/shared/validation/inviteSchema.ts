...
export const sendWorkspaceInviteSchema = z.object({
  email: z.string().trim().email("Invalid email address").toLowerCase(),
  firstName: z
    .string()
    .trim()
  ...
  workspaceId: z
    .string()
    .trim()
    .uuid({ message: "Invalid workspace id format" }),
});

export type sendWorkspaceInviteInput = z.infer<
  typeof sendWorkspaceInviteSchema
>;
