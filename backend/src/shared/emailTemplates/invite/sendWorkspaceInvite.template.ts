export interface SendWorkspaceInviteProps {
  firstName: string;
  inviterName: string;
  companyName: string;
...
export const sendWorkspaceInviteTemplate = (
  props: SendWorkspaceInviteProps,
) => {
  const {
    firstName,
    inviterName,
  ...
  return {
    subject: `You've been invited to join ${workspaceName} on Pivot`,
    html: `
      <!DOCTYPE html>
      <html>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p>
            ...
            <strong>${workspaceName}</strong> workspace in
